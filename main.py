...
def run_agent():
    # Gemini設定: モデル名を 'gemini-1.5-flash' に修正
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Geminiで分析（モデル名の修正によりここが動くようになります）
            print("Geminiがデータを直接分析しています...")
            prompt = f"以下のMEOダッシュボードのテキストデータから、主要な数値（検索数、閲覧数、アクション数など）を抜き出し、日本語で簡潔に報告してください。\n\nデータ内容:\n{page_content}"
            response = model.generate_content(prompt)

            # GASへ送信
            print(f"分析結果を送信します: {response.text[:50]}...")
            requests.post(os.environ["GAS_URL"], json={"message": response.text})
            print("スプレッドシートへの送信が完了しました！成功です！")
